import { getAudioContext, getMasterGain, soundManager } from './core.js';
import { initializeAudio, getSynthesizer } from '../../audio/index.js';
...
(function() {
    ...
    function apply() {
        ...
        function createDirectTone(frequency, volume, duration = 0.1) {
            const audioCtx = getAudioContext(); // Use imported function
            if (!audioCtx) {
                console.warn("DirectTone: Audio context not available.");
                return;
            }
            // Get the master gain from core.js soundManager
            const coreMasterGain = getMasterGain();
            const destination = coreMasterGain || audioCtx.destination; // Prefer connecting to the central master gain
            ...
            osc.connect(gain);
            gain.connect(destination); // Connect to the correct destination
